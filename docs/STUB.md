...
## Shipped (was parked)
- **Journaling** — DONE. Personal + teacher-assigned journal prompts at `/dashboard/journal` (student route); `lib/journal/journal.ts` builds the metadata-only submission record (`wordCount`/`secondsSpent`/`status`) so entry content never leaves `users/{uid}/journal`. Teacher assigns via the Course page (`type:'journal'`), sees per-student status on the student detail page. **Needs the updated Firestore rules deployed** (`docs/firestore.rules.proposed` — new `classes/{classId}/assignments/{assignmentId}/submissions/{studentUid}` rule; the existing `users/{userId}/{document=**}` owner rule already covers journal entry content, no change needed there).
