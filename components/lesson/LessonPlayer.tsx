      ...
      <div className="fixed top-0 left-0 right-0 z-20 bg-bgSage/95 backdrop-blur flex items-center justify-between" style={{ padding: '20px 32px' }}>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-full text-textTitle/40 hover:text-textTitle hover:bg-white/60 transition"
          >
            <FlagIcon />
          </button>
          <button
            onClick={onExit}
            className="flex items-center gap-2 text-sm text-textTitle/50 hover:text-textTitle px-3 py-1.5 rounded-full hover:bg-white/60 transition"
          >
      ...
      <div className="bg-white rounded-3xl shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
        {sent ? (
          ...
          <>
            ...
            <p className="text-xs text-textTitle/50 mb-4">Lesson {lesson} · slide {slide}</p>
            <textarea
...
function SlideView({ slide, onAnswered, requireCorrect }: { slide: Slide; onAnswered: () => void; requireCorrect: boolean }) {
  switch (slide.type) {
    case 'title':
      return (
        <div className="text-center">
          ...
          {slide.detailText && <p className="text-textTitle/50 text-lg md:text-xl">{slide.detailText}</p>}
        </div>
      )
    case 'objectives':
      return (
        <div>
          ...
          {slide.subheader && <p className="text-textTitle/60 text-lg mb-6">{slide.subheader}</p>}
    ...
    case 'reflectPrompt':
      return (
        <div className="text-center">
          <div className="text-sm uppercase tracking-wider text-textTitle/40 mb-3">{slide.mainTitle ?? 'Reflect & Think'}</div>
    ...
    case 'recap':
      return (
        <div className="text-center">
          <SlideImage src={slide.image} />
          {slide.eyebrow && <div className="text-sm uppercase tracking-wider text-textTitle/40 mb-2">{slide.eyebrow}</div>}
            ...
            {slide.items.map((it, i) => (
              <div key={i} className="flex items-start gap-3 bg-white rounded-2xl p-5 shadow-sm">
                ...
                <div>
                  ...
                  {it.description && <div className="text-textTitle/60">{it.description}</div>}
                </div>
              </div>
            ))}
          </div>
    ...
    case 'stepByStep':
      return (
        <div>
          {slide.eyebrow && <div className="text-sm uppercase tracking-wider text-textTitle/40 mb-1 text-center">{slide.eyebrow}</div>}
          <h2 className="font-display text-2xl md:text-3xl text-textTitle mb-1 text-center">{slide.title}</h2>
          {slide.subtitle && <p className="text-textTitle/60 text-lg mb-6 text-center">{slide.subtitle}</p>}
            ...
            <div>
              ...
              {slide.pros.map((p, i) => (
                <div key={i} className="bg-white rounded-2xl p-4 shadow-sm mb-2">
                  ...
                  {p.description && <div className="text-textTitle/60 text-sm">{p.description}</div>}
                </div>
              ))}
            </div>
            <div>
              ...
              {slide.cons.map((c, i) => (
                <div key={i} className="bg-white rounded-2xl p-4 shadow-sm mb-2">
                  ...
                  {c.description && <div className="text-textTitle/60 text-sm">{c.description}</div>}
                </div>
              ))}
            </div>
          </div>
    ...
    case 'contextualComparison':
      return (
        <div>
          ...
          {slide.footer && <p className="text-center text-textTitle/50 text-sm mt-4">{slide.footer}</p>}
        </div>
      )
    case 'visualAnalogy':
      return (
        <div className="text-center">
          ...
          {slide.subtitle && <p className="text-textTitle/60 mb-6">{slide.subtitle}</p>}
          <div className="grid gap-3">
            {slide.contexts.map((c, i) => (
              <div key={i} className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                {c.text && <div className="text-textTitle/60 text-sm">{c.text}</div>}
              </div>
            ))}
          </div>
        </div>
    ...
    case 'image':
      return (
        <div className="text-center">
          ...
          {slide.caption && <p className="text-textTitle/60">{slide.caption}</p>}
        </div>
      )
    ...
    case 'calloutQuote':
      return (
        <div className="text-center">
          ...
          {slide.author && <p className="text-textTitle/50 mt-3">— {slide.author}</p>}
        </div>
      )
    case 'callToAction':
      return (
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl p-5 shadow-sm border-l-4 border-textTitle/20">
              <div className="text-xs uppercase tracking-wider text-textTitle/40 mb-2">Before</div>
...
function TrueFalse({ slide, onAnswered, requireCorrect }: { slide: Extract<Slide, { type: 'trueFalse' }>; onAnswered: () => void; requireCorrect: boolean }) {
  ...
  return (
    <div className="text-center">
      <div className="text-sm uppercase tracking-wider text-textTitle/40 mb-3">True or False</div>
...
function MatchConcept({ slide, onAnswered }: { slide: Extract<Slide, { type: 'matchConcept' }>; onAnswered: () => void }) {
  ...
  return (
    <div>
            ...
            return (
              <button key={c} onClick={() => !done && setPickedConcept(active ? null : c)} disabled={done}
                className={`w-full text-left px-4 py-3 rounded-2xl shadow-sm text-sm transition ${
                  done ? 'bg-brandGreen/15 text-textTitle/50' : active ? 'bg-brandGreen text-white' : 'bg-white text-textTitle hover:bg-white/70'
                }`}>
                {c}
              </button>
            )
          ...
          {shuffledDefs.current.map((d) => {
            ...
            return (
              <button key={d} onClick={() => chooseDef(d)} disabled={done || !pickedConcept}
                className={`w-full text-left px-4 py-3 rounded-2xl shadow-sm text-sm transition ${
                  done ? 'bg-brandGreen/15 text-textTitle/50' : isWrong ? 'bg-red-500 text-white' : 'bg-white text-textTitle hover:bg-white/70 disabled:opacity-60'
                }`}>
                {d}
              </button>
            )
      ...
      <p className="text-center text-xs text-textTitle/50 mt-4">
...
function InteractiveGrowthVisual({ slide, onAnswered }: { slide: Extract<Slide, { type: 'interactiveGrowthVisual' }>; onAnswered: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      ...
      {slide.subtitle && <p className="text-sm text-textTitle/60 text-center">{slide.subtitle}</p>}
      ...
      <div className="w-full max-w-md flex flex-col gap-4">
        ...
        <div className="text-xs text-textTitle/45 text-center">plus ${monthly}/mo, compounded monthly over {years} years</div>
      </div>
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-sm p-4">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
          ...
          <path d={path(late)} fill="none" className="text-textTitle/30" stroke="currentColor" strokeWidth={2.5} />
      ...
      <div className="text-center">
        ...
        <div className="text-sm text-textTitle/60">{slide.summaryMessage}</div>
      </div>
    </div>
  )
}
