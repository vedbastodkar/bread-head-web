    ...
    <main className="min-h-screen bg-bgSage pt-28 pb-20 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <section className="mt-6 rounded-3xl border border-textTitle/10 p-5" style={{ background: 'linear-gradient(180deg,#EEF2E4,#E1E9D0)' }}>
          ...
          {boxes.length === 0 ? (
            <p className="text-textTitle/45 text-sm py-4 text-center">Add your first bucket — give every dollar a job.</p>
          ) : (
  ...
  return (
    <div className="flex items-end gap-3 flex-wrap">
      <label className="flex flex-col gap-1">
        <span className="text-[10px] font-bold tracking-[0.1em] uppercase text-textTitle/40">Bucket name</span>
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {box.targetMode === 'percent' && (
            <span className="text-[12px] text-textTitle/50 tabular-nums whitespace-nowrap">= {money(dollars)}</span>
          )}
        </div>
      </div>
