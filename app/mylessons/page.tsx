...
export default function CourseOverview() {
  ...
  return (
    <StudentShell data={data!} user={user} signOut={signOut}>
      <Link href="/dashboard" className="text-sm text-textTitle/50 hover:text-textTitle">← Dashboard</Link>
      <h1 className="font-display text-3xl text-textTitle mt-1 mb-2">Personal Finance</h1>
      <p className="text-textTitle/60 mb-8 max-w-2xl">
            ...
            <div key={u.unit} className="bg-white rounded-2xl shadow-sm p-6">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="max-w-xl">
                  ...
                  <p className="text-textTitle/60 text-sm">{u.description}</p>
                  {(() => {
                    const objs = unitObjectives(u.unit).slice(0, 3)
                    return objs.length > 0 ? (
                      <p className="text-xs text-textTitle/45 mt-2">You’ll learn: {objs.join(' · ')}</p>
                    ) : null
                  })()}
                </div>
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                <span className="text-xs text-textTitle/50">{doneCount}/{u.lessonCount} lessons</span>
              </div>
            </div>
          )
        })}
