      ...
      const assignments = await Promise.all(assignSnap.docs.map(async (a) => {
        ...
        try {
          ...
          subSnap.forEach((s) => {
            ...
            const submittedAt = sa && typeof sa.toDate === 'function' ? sa.toDate().toISOString() : null
            submissions![s.id] = isJournal
              ? {
              ...
              : {
                  completedLessonIds: (s.get('completedLessonIds') ?? []) as string[],
                  status: s.get('status') ?? 'in_progress',
                  submittedAt,
                }
          })
