...
export function TeacherHome() {
  ...
  return (
    <DashboardShell data={data!} user={user} signOut={signOut} reload={reload}>
      ...
      <p className="text-textTitle/60 text-sm mb-6">Everything across your classes at a glance.</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {flagged.map(({ s, cls, flags }) => (
              <Link key={`${cls.id}-${s.uid}`} href={`/dashboard/${cls.id}/${s.uid}`}
                ...
                <div className="text-xs text-textTitle/50 mb-2">{cls.name}</div>
        ...
        {(['active', 'archived'] as const).map((k) => {
          ...
          return (
            <button key={k} onClick={() => setShowArchived(k === 'archived')}
              className={`px-3 py-1 rounded-lg ${on ? 'bg-brandGreen text-white' : 'text-textTitle/60 hover:bg-white'}`}>
      ...
      {cards.length === 0 && (
        <p className="text-textTitle/60">{showArchived ? 'No archived classes.' : 'No classes yet — use “New class”.'}</p>
      )}
...
function AggStat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm p-4">
      ...
      <div className="text-textTitle/50 text-xs mt-1">{label}</div>
    </div>
  )
}
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div>
      ...
      <div className="text-textTitle/50 text-xs">{label}</div>
    </div>
  )
}
