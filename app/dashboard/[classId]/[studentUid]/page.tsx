...
export default function StudentDetail() {
  ...
  if (!cls || !s) return <DashboardLoading><p className="text-textTitle/70">Student not found.</p></DashboardLoading>
  ...
  return (
    <DashboardShell data={data!} activeClassId={cls.id} user={user} signOut={signOut} reload={reload}>
      <Link href={`/dashboard/${cls.id}`} className="text-sm text-textTitle/70 hover:text-textTitle">← {cls.name}</Link>
      ...
      {lessonAssignments.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-[11px] text-textTitle/70">Completion counts only lessons finished while assigned</div>
          </div>
              ...
              return (
                <div key={a.id} className="flex items-center justify-between text-sm border-b border-textTitle/5 pb-2 last:border-0">
                  <div className="min-w-0">
                    ...
                    {a.dueDate && <div className="text-xs text-textTitle/70">Due {a.dueDate}</div>}
                  </div>
                  <div className="text-right shrink-0">
                    <div className={`text-xs font-medium ${done ? 'text-brandGreen' : inProgress ? 'text-accentGold' : 'text-textTitle/70'}`}>
                      {done ? 'Complete' : inProgress ? 'In progress' : 'Not started'}
                    </div>
                    {total > 1 && <div className="text-[11px] text-textTitle/70">{doneCount}/{total} lessons</div>}
                  </div>
                </div>
              )
            })}
            ...
            {journalAssignments.map((a) => {
              ...
              return (
                <div key={a.id} className="flex items-center justify-between text-sm border-b border-textTitle/5 pb-2 last:border-0">
                  ...
                  <div className="text-right shrink-0">
                    <div className={`text-xs font-medium ${done ? 'text-brandGreen' : sub ? 'text-accentGold' : 'text-textTitle/70'}`}>
                      {done ? 'Complete' : sub ? 'In progress' : 'Not started'}
                    </div>
                    {sub && <div className="text-[11px] text-textTitle/70">{sub.wordCount ?? 0} words · {Math.floor((sub.secondsSpent ?? 0) / 60)}m</div>}
                  </div>
                </div>
              )
            })}
            ...
            <div key={u.unit} className="bg-white rounded-2xl shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="font-medium text-textTitle">
                  <span className="text-textTitle/70 mr-2">U{u.unit}</span>{u.name}
                </div>
                <div className="text-xs text-textTitle/70">{done}/{u.lessonCount}</div>
              </div>
      ...
      <div className="flex items-center gap-3 mt-5 text-xs text-textTitle/70">
