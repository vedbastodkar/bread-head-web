  ...
  return (
    <StudentShell data={data!} user={user} signOut={signOut}>
      ...
      <section className="mb-6">
        <h2 className="text-xs font-semibold tracking-wider text-textTitle/70 uppercase mb-2">
        ...
        {assigned.length === 0 && challengeTodo.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/70">Nothing assigned right now. You’re all caught up.</div>
        ) : (
            ...
            {assigned.map((a) => {
              ...
              return (
                <Link key={a.id} href={`/mylessons/${p.unit}/${p.lesson}`}
                  ...
                  <div className="text-xs text-textTitle/70">{unitName(p.unit)}</div>
                  {a.dueDate && <div className={`text-xs mt-1 ${a.overdue ? 'text-red-600' : 'text-textTitle/70'}`}>{a.overdue ? 'Overdue' : 'Due'} {a.dueDate}</div>}
                </Link>
              )
            })}
            {challengeTodo.map((c) => {
      ...
      <div className="rounded-2xl shadow-sm overflow-hidden bg-white mb-4">
        ...
        <div className="px-6 py-3 flex items-center gap-3">
          ...
          <span className="text-xs text-textTitle/70">{completed.size} / {TOTAL_LESSONS} lessons</span>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs text-textTitle/70">{unitDone}/{u.lessonCount}</span>
          </div>
...
function StatTile({ label, value, accent }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5 text-center">
      {/* accent uses the deeper gold (#9c7d1f) not accentGold — bright gold on white
          is ~2.2:1 and fails WCAG; the deeper gold clears large-text AA and stays gold. */}
      <div className={`font-display text-3xl mb-1 ${accent ? 'text-[#9c7d1f]' : 'text-textTitle'}`}>{value}</div>
      <div className="text-xs uppercase tracking-wider text-textTitle/70">{label}</div>
    </div>
  )
}
      ...
      <section>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 flex items-center justify-between gap-4 flex-wrap">
          <div className="max-w-sm">
            ...
            <p className="text-sm text-textTitle/70">Join your teacher’s section by entering their section code.</p>
          </div>
        ...
        {msg && <p className="text-sm text-textTitle/70 mt-2">{msg}</p>}
      </section>

      <section>
        <h2 className="font-display text-2xl text-textTitle mb-1">Classroom Sections</h2>
        <p className="text-sm text-textTitle/70 mb-4 max-w-2xl">Sections you have joined. Your teacher can see your course progress and reset your password if you forget it.</p>
        {sections === null ? (
          <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/70">Loading…</div>
        ) : activeSections.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/70">You haven’t joined a section yet. Enter your teacher’s code above.</div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-textTitle/70 border-b border-textTitle/10 bg-bgSage/40">
