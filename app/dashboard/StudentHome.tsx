  ...
  return (
    <StudentShell data={data!} user={user} signOut={signOut}>
      ...
      <section className="mb-6">
        <h2 className="text-xs font-semibold tracking-wider text-textTitle/65 uppercase mb-2">
        ...
        {assigned.length === 0 && challengeTodo.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/65">Nothing assigned right now. You’re all caught up.</div>
        ) : (
            ...
            {assigned.map((a) => {
              ...
              return (
                <Link key={a.id} href={`/mylessons/${p.unit}/${p.lesson}`}
                  ...
                  <div className="text-xs text-textTitle/65">{unitName(p.unit)}</div>
                  {a.dueDate && <div className={`text-xs mt-1 ${a.overdue ? 'text-red-600' : 'text-textTitle/65'}`}>{a.overdue ? 'Overdue' : 'Due'} {a.dueDate}</div>}
                </Link>
              )
            })}
            {challengeTodo.map((c) => {
              ...
              return (
                <Link key={c.id} href={`/budgetchallenge/${c.id}`}
                  ...
                  {done ? (
                    ...
                    c.dueDate && <div className={`text-xs mt-1 ${c.overdue ? 'text-red-600' : 'text-textTitle/65'}`}>{c.overdue ? 'Overdue' : 'Due'} {c.dueDate}</div>
                  )}
      ...
      <div className="rounded-2xl shadow-sm overflow-hidden bg-white mb-4">
        ...
        <div className="px-6 py-3 flex items-center gap-3">
          ...
          <span className="text-xs text-textTitle/65">{completed.size} / {TOTAL_LESSONS} lessons</span>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col">
          <div className="text-xs uppercase tracking-wider text-textTitle/65 mb-2">Current unit</div>
          <div className="font-display text-xl text-textTitle mb-1">Unit {u.unit} — {u.name}</div>
          <p className="text-textTitle/65 text-sm mb-4">{u.description}</p>
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1 h-2 rounded-full bg-bgSage overflow-hidden">
              <div className="h-full bg-brandGreen" style={{ width: `${unitPct}%` }} />
            </div>
            <span className="text-xs text-textTitle/65">{unitDone}/{u.lessonCount}</span>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col">
          <div className="text-xs uppercase tracking-wider text-textTitle/65 mb-2">Current lesson</div>
          <div className="font-display text-xl text-textTitle mb-1">Lesson {cont.lesson}: {lname}</div>
          <p className="text-textTitle/65 text-sm mb-4">
...
function StatTile({ label, value, accent }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5 text-center">
      ...
      <div className="text-xs uppercase tracking-wider text-textTitle/65">{label}</div>
    </div>
  )
}
      ...
      <section>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 flex items-center justify-between gap-4 flex-wrap">
          <div className="max-w-sm">
            ...
            <p className="text-sm text-textTitle/65">Join your teacher’s section by entering their section code.</p>
          </div>
        ...
        {msg && <p className="text-sm text-textTitle/65 mt-2">{msg}</p>}
      </section>

      <section>
        <h2 className="font-display text-2xl text-textTitle mb-1">Classroom Sections</h2>
        <p className="text-sm text-textTitle/65 mb-4 max-w-2xl">Sections you have joined. Your teacher can see your course progress and reset your password if you forget it.</p>
        {sections === null ? (
          <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/65">Loading…</div>
        ) : activeSections.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/65">You haven’t joined a section yet. Enter your teacher’s code above.</div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-textTitle/65 border-b border-textTitle/10 bg-bgSage/40">
