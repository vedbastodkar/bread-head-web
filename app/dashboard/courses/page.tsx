...
export default function AllCourses() {
  ...
  return (
    <DashboardShell data={data!} user={user} signOut={signOut} reload={reload}>
      ...
      <p className="text-textTitle/60 text-sm mb-8">Every class you teach, current and archived.</p>

      <Section title="Current">
        {current.length === 0 && <Empty>No current classes.</Empty>}
        {current.map((c) => (
          <Row key={c.id} cls={c} action={
            <button onClick={() => setArchived(c.id, true)} className="text-xs text-textTitle/40 hover:text-textTitle">Archive</button>
          } />
        ))}
      </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-8">
      <h2 className="text-xs font-semibold tracking-wider text-textTitle/40 uppercase mb-2">{title}</h2>
...
function Empty({ children }: { children: React.ReactNode }) {
  return <div className="px-5 py-4 text-sm text-textTitle/40">{children}</div>
}
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-4 ${muted ? 'opacity-70' : ''}`}>
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-textTitle/50 mt-0.5">
          {students} student{students !== 1 ? 's' : ''}
          {cls.joinCode && <> · Join {cls.joinCode}</>}
          {cls.grade?.length > 0 && <> · Grade {cls.grade.join(', ')}</>}
        </div>
      </div>
      <div className="hidden sm:block text-sm text-textTitle/60 w-24 text-right">{avg}% avg</div>
      <div className="hidden sm:block text-sm w-28 text-right">
        {needHelp > 0 ? <span className="text-red-600">{needHelp} need help</span> : <span className="text-textTitle/40">on track</span>}
      </div>
