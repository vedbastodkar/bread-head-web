...
export default function LessonsContentPage() {
  ...
  return (
    <DashboardShell data={data!} user={user} signOut={signOut} reload={reload}>
      ...
      <p className="text-textTitle/70 text-sm mb-6">Browse the curriculum and assign lessons to a class or specific students.</p>
            ...
            return (
              <div key={u.unit} className="bg-white rounded-2xl shadow-sm">
                <button onClick={() => toggleUnitOpen(u.unit)} className="w-full flex items-center justify-between p-4 text-left">
                  <span className="font-medium text-textTitle"><span className="text-textTitle/70 mr-2">U{u.unit}</span>{u.name}</span>
                  <span className="text-xs text-textTitle/70">{chosen > 0 ? `${chosen} selected` : `${u.lessonCount} lessons`} {open ? '▾' : '▸'}</span>
                </button>
                {open && (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {lessonSummary(u.unit, i + 1) && (
                                <div className="text-xs text-textTitle/70 truncate">{lessonSummary(u.unit, i + 1)}</div>
                              )}
                            </div>
                            <button
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl shadow-sm p-5 lg:sticky lg:top-28">
            <div className="text-sm font-medium text-textTitle mb-1">
              {editing ? 'Edit assignment' : 'Assign'} {selected.size > 0 && <span className="text-textTitle/70">· {selected.size} lesson{selected.size > 1 ? 's' : ''}</span>}
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-5 mt-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-xs text-textTitle/70 max-w-md">Release the curriculum gradually and set default in-lesson rules for a class. Assignments can override these per lesson or per student.</p>
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6 mt-4">
          ...
          <div className="space-y-2 text-sm text-textTitle/80">
            <div className="text-xs uppercase tracking-wider text-textTitle/70">Default lesson controls</div>
            <label className="flex items-center gap-2">
            ...
            {lesson ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl shadow-xl max-w-md w-full p-8 text-center" onClick={(e) => e.stopPropagation()}>
                  <p className="text-sm text-textTitle/70 mb-4">Lesson content unavailable.</p>
