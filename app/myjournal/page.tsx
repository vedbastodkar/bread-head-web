    ...
    <StudentShell data={data!} user={user} signOut={signOut}>
      ...
      {journalAssignments.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xs font-semibold tracking-wider text-textTitle/70 uppercase mb-2">Assigned prompts</h2>
            ...
            {journalAssignments.map((a) => {
              ...
              return (
                <button key={a.id} onClick={() => openAssigned(a)}
                  ...
                  <div className="text-xs text-textTitle/70 mt-0.5 line-clamp-2">{a.journal?.questions[0]}</div>
                  <div className="text-xs mt-1 flex gap-2">
                    {entry ? <span className="text-brandGreen">Started · {entry.wordCount} words</span> : <span className="text-textTitle/70">Not started</span>}
                    {a.dueDate && <span className={overdue ? 'text-red-600' : 'text-textTitle/70'}>{overdue ? 'Overdue' : 'Due'} {a.dueDate}</span>}
                    {a.journal && (a.journal.minWords > 0 || a.journal.minSeconds > 0) && (
                      <span className="text-textTitle/70">min {a.journal.minWords}w{a.journal.minSeconds ? ` · ${a.journal.minSeconds}s` : ''}</span>
                    )}
                  </div>
                </button>
              )
      ...
      <h2 className="text-xs font-semibold tracking-wider text-textTitle/70 uppercase mb-2">Past entries</h2>
      {loading ? (
        <div className="bg-white rounded-2xl shadow-sm p-4 text-sm text-textTitle/70">Loading…</div>
      ) : entries.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm p-6 text-sm text-textTitle/70">
          ...
          {entries.map((e) => (
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-textTitle/70">
                  {e.lastModified ? e.lastModified.toLocaleDateString() : 'Draft'} · {e.wordCount} words
                </div>
              </div>
              <span className="text-textTitle/70 text-sm shrink-0">Edit ›</span>
            </button>
          ))}
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-xl max-w-2xl w-full p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="text-xs text-textTitle/70">{words} words · {Math.floor(seconds / 60)}m {seconds % 60}s</div>
