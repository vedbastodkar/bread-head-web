  ...
  return (
    <div className="min-h-[70vh] flex flex-col">
      ...
      <div className="max-w-2xl mx-auto w-full mt-10 flex items-center gap-3">
        {i > 0 && (
          <button onClick={back} disabled={busy} className="text-sm font-semibold text-textTitle/70 hover:text-textTitle px-4 py-3 disabled:opacity-50">← Back</button>
        )}
        {i <= CONCEPT_LAST && (
          <button onClick={skipIntro} className="text-sm text-textTitle/70 hover:text-textTitle underline">Skip intro</button>
        )}
...
function Sub({ children }: { children: React.ReactNode }) {
  return <p className="text-textTitle/70 text-[15px] leading-relaxed mt-3 max-w-[52ch]">{children}</p>
}
        ...
        {items.map((it) => (
          <div key={it.n} className="flex items-start gap-3.5 bg-white border border-textTitle/10 rounded-xl px-4 py-3.5">
            ...
            <div>
              ...
              <p className="text-[13px] text-textTitle/70">{it.d}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        {BUDGET_TEMPLATES.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => onSelect(t.id)} aria-pressed={on} type="button"
              ...
              <p className="text-[13px] text-textTitle/70 mt-1.5 leading-relaxed">{t.description}</p>
      ...
      <div className="flex flex-col gap-3 mt-6">
        <div className="flex items-center justify-between gap-4 bg-white border border-textTitle/10 rounded-2xl p-4">
          <div>
            ...
            <p className="text-[13px] text-textTitle/70">Move unspent money into savings at month’s end.</p>
          </div>
        ...
        <div className="flex items-center justify-between gap-4 bg-white border border-textTitle/10 rounded-2xl p-4">
          <div>
            ...
            <p className="text-[13px] text-textTitle/70">When to nudge you to review your spending.</p>
          </div>
        ...
        <div className="flex items-center justify-between gap-4 bg-white border border-textTitle/10 rounded-2xl p-4">
          <div>
            ...
            <p className="text-[13px] text-textTitle/70">Auto-save this % of new income.</p>
          </div>
          <div className="flex items-center gap-1">
            ...
            <span className="text-textTitle/70 text-sm">%</span>
          </div>
        </div>
      </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between text-[14px]">
      <span className="text-textTitle/70">{label}</span>
